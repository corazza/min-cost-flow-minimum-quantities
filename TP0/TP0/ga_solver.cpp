...
Flow mutate(const Network &network, const Flow &original, std::set<edge_key> active_vlbs, int num_perturbations) {
    ...
    for (int i = 0; i < num_perturbations; ++i) {
        std::vector<vertex_key> source_to_v_from = find_random_augmenting_path(network, flow, active_vlbs, network.source, network.sink, -1, false); // TODO FIXME see below
        apply_augmenting_path(network, flow, active_vlbs, source_to_v_from, -1);
        std::vector<vertex_key> v_to_to_sink = find_random_augmenting_path(network, flow, active_vlbs, network.source, network.sink, 1, false);
        apply_augmenting_path(network, flow, active_vlbs, v_to_to_sink, 1);        
    }
